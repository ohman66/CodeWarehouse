$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sysCzjlb/list',
        datatype: "json",
        colModel: [
			{ label: 'ID', name: 'id', index: "id", width: 45, key: true },
			{ label: '充值人员', name: 'username', index: "qcmc", width: 75 },
			{ label: '充值金额', name: 'jine', index: "qcmc", width: 75 },
			{ label: '充值时间', name: 'createtime', index: "qcmc", width: 75 },
        ],
		viewrecords: true,
        height: 385,
        rowNum: 10,
...
var vm = new Vue({
	...
	methods: {
		...
		add: function(){
			...
			vm.equipmentList = {};
			vm.title = "新增";
			vm.czjlb = {deptName:null, deptId:null};
			vm.getDept();
		},
		update: function () {
		...
		getCzjlb: function(czjlbId){
            $.get(baseURL + "sysCzjlb/info/"+czjlbId, function(r){
            	vm.czjlb = r.czjlb;
				vm.getDept();
    		});
		},
		saveOrUpdate: function () {
			var url = vm.czjlb.id == null ? "sysCzjlb/save" : "sysCzjlb/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.czjlb),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(){
							vm.reload();
        ...
        getDept: function(){
            ...
            $.get(baseURL + "sys/dept/list", function(r){
                ...
                var node = dept_ztree.getNodeByParam("deptId", vm.czjlb.deptid);
                if(node != null){
                    dept_ztree.selectNode(node);

                    vm.czjlb.deptName = node.name;
                }
            })
        },
        deptTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.czjlb.deptid = node[0].deptId;
                    vm.czjlb.deptName = node[0].name;
