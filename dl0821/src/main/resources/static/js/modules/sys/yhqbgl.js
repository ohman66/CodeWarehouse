$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sysYhqbgl/list',
        datatype: "json",
        colModel: [
			{ label: 'ID', name: 'id', index: "id", width: 45, key: true },
			{ label: '所属用户', name: 'username', index: "qcmc", width: 75 },
			{ label: '余额', name: 'yue', index: "qcmc", width: 75 },
...
var vm = new Vue({
	...
	methods: {
		...
		add: function(){
			...
			vm.equipmentList = {};
			vm.title = "新增";
			vm.yhqbgl = {deptName:null, deptId:null};
			vm.getDept();
		},
		update: function () {
		...
		getYhqbgl: function(yhqbglId){
            $.get(baseURL + "sysYhqbgl/info/"+yhqbglId, function(r){
            	vm.yhqbgl = r.yhqbgl;
				vm.getDept();
    		});
		},
		saveOrUpdate: function () {
			var url = vm.yhqbgl.id == null ? "sysYhqbgl/save" : "sysYhqbgl/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.yhqbgl),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(){
							vm.reload();
        ...
        getDept: function(){
            ...
            $.get(baseURL + "sys/dept/list", function(r){
                ...
                var node = dept_ztree.getNodeByParam("deptId", vm.yhqbgl.deptid);
                if(node != null){
                    dept_ztree.selectNode(node);

                    vm.yhqbgl.deptName = node.name;
                }
            })
        },
        deptTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.yhqbgl.deptid = node[0].deptId;
                    vm.yhqbgl.deptName = node[0].name;
